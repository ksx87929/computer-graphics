	...
	while (ftell (l_file) < filelength (fileno (l_file))) //Loop to scan the whole file 
	{
		...
		switch (l_chunk_id)
        {
			...
			case 0x4120:
				...
				{
					fprintf( stdout, "Number of polygons too high!\n");
					return (0);
				}				
                for (i=0; i<l_qty; i++)
                {
			...
			case 0x4140:
				...
				for (i=0; i<l_qty; i++)
				{
					...
					if(LOAD3DS_DEBUG) 
					{
						fprintf( stdout,"Mapping list u: %f\n",p_object->mapcoord[i].u);
						fprintf( stdout,"Mapping list v: %f\n",p_object->mapcoord[i].v);
					}
				}
                break;
